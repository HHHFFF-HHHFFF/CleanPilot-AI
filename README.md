...
面向扫地机器人的多工具智能客服 Agent。项目基于 LangChain `create_agent` 构建，集成通义千问、Chroma 向量检索、外部用户记录、动态提示词与 Streamlit 对话界面。

## 核心能力

- **多工具 Agent**：由 Qwen 自主选择并调用客服工具完成任务。
- **RAG 检索**：加载 TXT/PDF 知识文档，切分、向量化并写入 Chroma。
- **客服工具**：提供知识库问答、天气、用户位置、用户 ID、当前月份和外部使用记录查询。
- **动态提示词**：根据报告生成场景自动切换系统提示词和报告提示词。
- **工具中间件**：记录工具调用参数、结果与失败信息，并注入报告场景上下文。
- **Streamlit 前端**：提供浏览器端多轮对话与流式回答展示。
...
浏览器会自动打开本地会话页面。PyCharm 中建议将项目根目录标记为 **Sources Root**。
